...
function test_once() {
  ...
  script /tmp/special-qemu-riscv-nsh.log \
    $HOME/riscv/nuttx-riscv64/special-qemu-riscv-nsh.sh \
    $commit
  cat /tmp/special-qemu-riscv-nsh.log \
    | tr -d '\r' \
    | tr -d '\r' \
    | sed 's/\x08/ /g' \
    | sed 's/\x1B(B//g' \
    ...
    | cat -v \
    >/tmp/special-qemu-riscv-nsh-clean.log
  cat /tmp/special-qemu-riscv-nsh-clean.log | \
    gh gist create \
    --public \
    --desc "$commit Special Build and Test NuttX for QEMU RISC-V 32-bit (Flat Build)" \
    --filename "special-qemu-riscv-nsh.log"
}

## Test 20 times
function stress_test() {
  test_once
  cd /tmp/special-qemu-riscv-nsh64/nuttx/nuttx
  for i in {1..20}
  do
    echo "===== Attempt $i"
...
commit=$1 ## Optional Commit ID
tmp_path=/tmp/special-qemu-riscv-nsh
rm -rf $tmp_path
mkdir $tmp_path
cd $tmp_path
neofetch

# name: special-qemu-riscv-nsh (RV32 Kernel Build, Special Build and Test)
...
#       run:  |
    ...
    #   with:
    #     tag_name: special-qemu-riscv-nsh-${{ steps.date.outputs.date }}
    #     draft: false
    #     prerelease: false
