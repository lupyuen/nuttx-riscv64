...
echo "***** Now running test_once in https://github.com/lupyuen/nuttx-riscv64/blob/main/special-qemu-riscv-knsh64.sh"

function test_once() {
  commit=$1 ## Optional Commit ID
  script /tmp/special-qemu-riscv-knsh64.log \
    $HOME/riscv/nuttx-riscv64/special-qemu-riscv-knsh64.sh \
    $commit
  ...
  cat /tmp/special-qemu-riscv-knsh64-clean.log | \
    gh gist create \
    --public \
    --desc "$commit Special Build and Test NuttX for QEMU RISC-V 64-bit (Kernel Build)" \
...
# name: special-qemu-riscv-knsh64 (RV64 Kernel Build, Special Build and Test)
...
#       run:  |
    ...
    #   with:
    #     tag_name: special-qemu-riscv-knsh64-${{ steps.date.outputs.date }}
    #     draft: false
    #     prerelease: false
